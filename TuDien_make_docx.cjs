...
const data = JSON.parse(fs.readFileSync(path.join(__dirname, "dictionary.json"), "utf8"));
const OUT = path.resolve(__dirname, "..", "..", "TuDien_TiengAnh_THCS_Lop6-9.docx");
...
const cover = [
  ...spacer(6),
  P(R("TỪ ĐIỂN TỪ VỰNG TIẾNG ANH THCS", { bold: true, size: 48, color: "1F3864" }), { align: AlignmentType.CENTER, after: 240 }),
  P(R("Lớp 6 – 9  •  Bộ sách Global Success", { size: 30, color: "2E5496" }), { align: AlignmentType.CENTER, after: 120 }),
...
const doc = new Document({
  ...
  sections: [
    { properties: { page: { size: { width: 11906, height: 16838 }, margin: { top: 720, bottom: 720, left: 600, right: 600 } } }, children: [...cover, ...guide] },
    {
      ...
      headers: { default: new Header({ children: [P([R("Từ điển Từ vựng Tiếng Anh THCS (Lớp 6–9)", { size: 16, color: "999999" })], { align: AlignmentType.CENTER })] }) },
