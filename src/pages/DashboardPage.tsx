...
import { getProgress, getUserProfile, type UserProgress, type UserProfile } from "@/lib/progress";
...
const DashboardPage = () => {
  ...
  useEffect(() => {
    ...
    Promise.all([getUserProfile(user.uid), getProgress(user.uid)])
      .then(([p, prog]) => {
        setProfile(p);
        setProgress(prog);
      })
  ...
  const streak = Math.min(progress?.quizzesDone || 0, 7); // simplified streak
  ...
  const quickActions = [
    ...
    { label: "Thi đua", icon: Trophy, gradient: "gradient-energy", path: "/progress" },
  ];
  ...
  return (
    <div className="min-h-screen gradient-hero">
      ...
      <div className="fixed inset-0 -z-10 overflow-hidden">
        <div className="absolute top-[-20%] left-[-10%] w-[60vw] h-[60vw] rounded-full bg-[hsl(270,70%,93%)] blur-[180px] opacity-50" />
        <div className="absolute bottom-[-10%] right-[-10%] w-[50vw] h-[50vw] rounded-full bg-[hsl(335,70%,93%)] blur-[160px] opacity-40" />
      </div>
      ...
      <div className="max-w-5xl mx-auto px-5 pt-28 pb-20">
        ...
        >
          <div className="flex items-center gap-4 mb-2">
            <img
              src={user.photoURL || ""}
              alt="Avatar"
              className="w-14 h-14 rounded-2xl border-3 border-primary/30 object-cover shadow-lg"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <motion.div
            ...
            className="bg-card/80 backdrop-blur-xl rounded-3xl p-6 border border-white/60 shadow-lg"
          >
              ...
              {[
                ...
                <motion.div
                  ...
                  className={`flex items-center justify-between p-3 rounded-xl transition-colors ${
                    item.done ? "bg-success/10" : "bg-muted/50 hover:bg-muted/80"
                  }`}
                >
            ...
            {recentQuizzes.length === 0 ? (
              <div className="text-center py-8">
                ...
                <p className="text-muted-foreground text-sm font-medium">
                  Em chưa làm bài nào. Bắt đầu học ngay nhé!
                </p>
              </div>
            ) : (
              <div className="space-y-3">
                {recentQuizzes.map((q, i) => {
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-xs font-bold text-white ${isGood ? "gradient-success" : "gradient-warm"}`}>
                          {pct}%
                        </div>
                        <div>
                          <span className="text-sm font-bold text-foreground">Lớp {q.grade} · Unit {q.unit}</span>
                          <p className="text-xs text-muted-foreground">
                            {q.score}/{q.total} câu đúng
                          </p>
                        </div>
                      </div>
        ...
        <motion.div
          ...
          className="mt-8 bg-card/80 backdrop-blur-xl rounded-3xl p-6 border border-white/60 shadow-lg"
        >
              ...
              return (
                <motion.button
                  ...
                  className={`flex-shrink-0 w-16 h-20 rounded-2xl flex flex-col items-center justify-center gap-1 border-2 transition-all duration-300 ${
                    isDone
                      ? "gradient-success text-white border-transparent shadow-md"
