  ...
  const handleSelect = (idx: number) => {
    ...
    setTimeout(() => {
      if (current < questions.length - 1) { setCurrent((c) => c + 1); setSelected(null); }
        ...
        setFinished(true);
        if (user) {
          const finalScore = newAnswers.filter((a, i) => a === questions[i].ans).length;
              ...
              {questions[current].opts.map((opt, i) => {
                ...
                return (
                  <motion.button key={i} whileTap={{ scale: 0.97 }}
                    ...
                    className={`w-full text-left rounded-2xl px-5 py-3.5 font-bold text-sm border transition-all ${
                      selected !== null
