...
import { ArrowLeft, FileText, CheckCircle2, XCircle, BookOpen, Eye, PenTool, Send, Trophy } from "lucide-react";
  ...
  return (
    <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
      ...
      <div className="flex items-center justify-center gap-2 bg-xp/10 border border-xp/20 rounded-2xl p-3 mb-6 max-w-xs mx-auto">
        ...
        <span className="font-display font-extrabold text-lg text-xp">+{score * 10} XP</span>
      </div>
        ...
        {reviewMode === "after" && (
          <div className="mt-4">
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className="bg-card border border-border shadow-1 rounded-2xl overflow-hidden"
            >
              <div className="px-5 py-3.5 font-display font-bold text-sm text-foreground border-b border-border">
                Xem lại đáp án
              </div>
              <div className="flex flex-col gap-2 p-4">
                {questions.map((question, i) => {
                  ...
                  return (
                    <div key={i} className={`rounded-xl p-3 border ${correct ? "border-success/30 bg-success/10" : "border-destructive/30 bg-destructive/10"}`}>
                      ...
                      {!correct && <p className="text-xs text-muted-foreground">Bạn trả lời: <span className="line-through text-destructive">{userAns || "(bỏ trống)"}</span></p>}
                      <p className="text-xs font-bold text-success">Đáp án: {question.answer[0]}</p>
                    </div>
                  );
                })}
              </div>
...
const Grade10ReadingPage = () => {
  ...
  const sections = [
    { key: "signs", label: "Đọc biển báo", icon: Eye, desc: `${readingData?.signs ? Object.values(readingData.signs).reduce((sum, ex) => sum + (ex.questions?.length || 0), 0) : 0} câu`, color: "bg-primary text-primary-foreground" },
    { key: "cloze", label: "Điền từ vào đoạn văn", icon: BookOpen, desc: `${readingData?.cloze?.length || 0} đoạn`, color: "bg-accent text-accent-foreground" },
    { key: "comprehension", label: "Đọc hiểu", icon: FileText, desc: `${readingData?.comprehension?.length || 0} bài`, color: "bg-accent2 text-accent2-foreground" },
    { key: "letterArranging", label: "Sắp xếp thư/email", icon: PenTool, desc: `${writingData?.letterArranging?.questions?.length || 0} câu`, color: "bg-success text-success-foreground" },
    { key: "paragraphArranging", label: "Sắp xếp đoạn văn", icon: PenTool, desc: `${writingData?.paragraphArranging?.questions?.length || 0} câu`, color: "bg-accent text-accent-foreground" },
    { key: "sentenceArranging", label: "Sắp xếp câu", icon: PenTool, desc: `${writingData?.sentenceArranging?.questions?.length || 0} câu`, color: "bg-primary text-primary-foreground" },
    { key: "sentenceRewriting", label: "Viết lại câu", icon: PenTool, desc: `${writingData?.sentenceRewriting?.questions?.length || 0} câu`, color: "bg-accent2 text-accent2-foreground" },
  ];
      ...
      <PageShell>
        <div className="max-w-3xl mx-auto px-5 pt-28 pb-20">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={smooth}
            className="bg-accent text-accent-foreground rounded-2xl p-6 mb-6 shadow-2">
            <FileText className="h-8 w-8 mb-2 opacity-80" />
            <h1 className="font-display font-extrabold text-2xl">Đọc hiểu & Viết</h1>
            <p className="text-accent-foreground/80 text-sm">Luyện các kỹ năng đọc và viết</p>
          </motion.div>
            ...
            {sections.map((sec, i) => (
              ...
              >
                <div className="flex items-center gap-3">
                  <div className={`${sec.color} p-2.5 rounded-xl`}><sec.icon className="h-5 w-5" /></div>
                  <div>
        ...
        <div>
          {exKeys.length > 1 && (
            <div className="flex gap-2 mb-4 flex-wrap">
              {exKeys.map((k, i) => (
                <button key={k} onClick={() => setActiveSubIdx(i)} className={`px-3 py-1.5 rounded-full text-xs font-bold transition-all ${i === activeSubIdx ? "bg-primary text-primary-foreground" : "bg-card border border-border text-muted-foreground"}`}>Phần {i + 1}</button>
              ))}
            </div>
          )}
      ...
      return (
        <div>
          {items.length > 1 && (
            <div className="flex gap-2 mb-4 flex-wrap">
              {items.map((it, i) => (
                <button key={i} onClick={() => setActiveSubIdx(i)} className={`px-3 py-1.5 rounded-full text-xs font-bold transition-all ${i === activeSubIdx ? "bg-primary text-primary-foreground" : "bg-card border border-border text-muted-foreground"}`}>{it.title?.substring(0, 20) || `Bài ${i + 1}`}</button>
              ))}
            </div>
          )}
          {item.title && <h3 className="font-display font-bold text-foreground mb-2">{item.title}</h3>}
          {item.instruction && <div className="bg-card shadow-1 rounded-2xl p-4 mb-4 text-sm text-foreground leading-relaxed border border-border italic">{item.instruction}</div>}
          {item.passage && <div className="bg-card shadow-1 rounded-2xl p-4 mb-4 text-base text-foreground leading-relaxed max-h-48 sm:max-h-64 overflow-y-auto border border-border whitespace-pre-line">{item.passage}</div>}
      ...
      return (
        <div>
          {items.length > 1 && (
            <div className="flex gap-2 mb-4 flex-wrap">
              {items.map((it, i) => (
                <button key={i} onClick={() => setActiveSubIdx(i)} className={`px-3 py-1.5 rounded-full text-xs font-bold transition-all ${i === activeSubIdx ? "bg-primary text-primary-foreground" : "bg-card border border-border text-muted-foreground"}`}>{it.title?.substring(0, 25) || `Bài ${i + 1}`}</button>
              ))}
            </div>
          )}
          {item.title && <h3 className="font-display font-bold text-foreground mb-2">{item.title}</h3>}
          {item.passage && <div className="bg-card shadow-1 rounded-2xl p-4 mb-4 text-base text-foreground leading-relaxed max-h-48 sm:max-h-60 overflow-y-auto border border-border">{item.passage}</div>}
          <MCQQuiz key={`${activeSubIdx}-${reviewMode}`} questions={item.questions} reviewMode={reviewMode} timeLimit={timeLimit} />
        </div>
      );
    }
