...
const Grade10VocabPage = () => {
  ...
  const resetQuiz = () => {
    setCurrent(0); setSelected(null); setSubmitted(false); setScore(0); setAnswers([]); setFinished(false); setQuizStarted(false);
  };
  ...
  // Compute score from answers array (for "after" mode and onTimeUp)
  ...
  if (finished) {
    ...
    // Build review questions for "after" mode
    ...
    if (reviewMode === "instant") {
      ...
      setTimeout(() => {
        if (current < questions.length - 1) {
          ...
          // Last question - compute score and finish
  ...
  // Show settings bar before quiz starts (first question, not yet submitted/selected in instant, or not yet started)
  const showSettings = current === 0 && !submitted && !quizStarted;
        ...
        <motion.div key={current} initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }}
          ...
          <div className="flex flex-col gap-2.5">
            {q.opts.map((opt, idx) => {
              ...
              return (
                <button key={idx} onClick={() => { if (!quizStarted) setQuizStarted(true); handleSelect(idx); }}
