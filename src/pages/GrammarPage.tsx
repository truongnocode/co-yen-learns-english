      ...
      <div className="max-w-3xl mx-auto w-full px-5 pt-12 pb-20">
        ...
        <motion.div initial={{ opacity: 0, y: 15, filter: "blur(6px)" }} animate={{ opacity: 1, y: 0, filter: "blur(0px)" }} transition={smooth}
          ...
          {unit.grammar.map((g, i) => (
            <span key={i} className="bg-primary text-primary-foreground text-xs font-bold px-3 py-1.5 rounded-full">{g}</span>
          ))}
        </motion.div>

        {/* Grammar notes */}
        <motion.div initial={{ opacity: 0, y: 20, filter: "blur(6px)" }} animate={{ opacity: 1, y: 0, filter: "blur(0px)" }} transition={{ ...smooth, delay: 0.1 }}
          className="bg-card rounded-2xl p-6 border border-border shadow-1 mb-8">
        ...
        <motion.button
          ...
          className="w-full btn-press bg-primary text-primary-foreground rounded-2xl py-4 font-display font-extrabold text-center shadow-1"
        >
