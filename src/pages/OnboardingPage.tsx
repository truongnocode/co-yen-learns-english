...
/**
 * Onboarding bắt buộc (2 bước): Tên + Lớp → SĐT phụ huynh.
 * v1 chỉ kiểm ĐỊNH DẠNG số VN (chưa OTP). Ghi đè lên tài khoản hiện tại,
 * giữ nguyên tiến trình. Prefill từ profile nếu đã có (vd tài khoản Google cũ).
 */
const OnboardingPage = () => {
  ...
  const handleFinish = async () => {
    ...
    setSaving(true);
    try {
      await completeOnboarding({ studentName: name.trim(), grade: grade as number, phone: normalized });
